...
from lighthouse.ui.functrace_overview import FunctionTraceOverview
...
class Lighthouse(object):
    ...
    def _init(self):
        ...
        # the coverage engine
        self.director = CoverageDirector(self,self.palette)
        ...
        self._ui_function_trace_overview=None
        # the directory to start the coverage file dialog in
        self._last_directory = None

        self._ui_exe_tree_overview = None
    ...
    def _uninstall_ui(self):
        ...
        self._uninstall_load_func_trace()
    ...
    def open_function_trace_overview(self):
        """
        ...
        if self._ui_function_trace_overview and self._ui_function_trace_overview.isVisible():
            self._ui_function_trace_overview.refresh()
            return

        # create a new coverage overview if there is not one visible
        self._ui_function_trace_overview = FunctionTraceOverview(self)
        self._ui_function_trace_overview.show()
    ...
    # Internal
    #--------------------------------------------------------------------------
    def interactive_load_func_trace(self,ctx):
        """
        Perform the user-interactive loading of individual coverage files.
        """
        self.palette.refresh_colors()
        ...
        disassembler.show_wait_box("Loading coverage from disk...")
        drcov_list = load_func_trace_files(filenames)
        if not drcov_list:
            disassembler.hide_wait_box()
            self.director.metadata.abort_refresh()
            return
        ...
        disassembler.replace_wait_box("Building database metadata...")
        await_future(future)
        self.director.org_function_trace_data=drcov_list;
        self.director.create_execute_trece_from_drcov_list(drcov_list)
        # insert the loaded drcov data objects into the director
        created_coverage, errors = self.director.create_coverage_from_drcov_list(drcov_list)
        self.director.create_function_trace_from_crcov(drcov_list)
        #
        # if the director failed to map any coverage, the user probably
        # provided bad files. emit any warnings and bail...
        #
        ...
        lmsg("Successfully loaded %u coverage file(s)..." % len(created_coverage))
        # self.open_coverage_overview()
        self.open_function_trace_overview()
...
# Warnings
#------------------------------------------------------------------------------
def load_func_trace_files(filenames):
    """
    Load multiple code coverage files from disk.
    """
    loaded_coverage = []
