...
class CoverageDirector(object):
    ...
    def __init__(self, core,palette):
        ...
        self.function_trace_data = False
        #
    ...
    def create_all_coverage(self, coverage_name, coverage_data, drcov_data):
        """
        Create a new database coverage mapping from the given data.
        """
        return self.update_coverage(coverage_name, coverage_data,  drcov_data.filepath,drcov_data)

    def refresh_execute_trace(self,_model):
        _model.title = "Function [" + self.get_main_module_func_name(self.exec_tree_addr)+"] Trace Record"
        retList=[]
        for funcov in  self.coverage.all_Excute_Function_list:
           if funcov.to_mod==self.main_module.id and funcov.to_address+self.metadata.imagebase ==self.exec_tree_addr:
               retList.append(funcov)
        self.build_Fun_cov_Name_List(retList)
        _model.main_data=retList

    def build_Fun_cov_Name_List(self, retList):
        for funcov in retList:
            self.build_Fun_cov_Name(funcov)
    ...
    def get_main_module_func_name(self,addr):
        return self.metadata.get_function(addr).name

    def get_org_memory_display(self,funcid,tp):
        for i, drcov_data in enumerate(self.org_function_trace_data, 1):
            for mem_data  in  drcov_data.memory_trace_list:
                if mem_data.funcid == funcid and mem_data.memorytype == tp:
                    return mem_data.memorydata
        return ''

    def rebuid__coverage_from_drcov_list(self, funcid_List):
        drcov_list=self.org_function_trace_data
        """
        ...
        for i, drcov_data in enumerate(drcov_list, 1):
            ...
            try:
                coverage_data = self._normalize_function_trace_data(drcov_data,funcid_List)
    ...
    def  create_function_trace_from_crcov(self,drcov_list):
        for i, drcov_data in enumerate(drcov_list, 1):
            self.function_trace_data= drcov_data.function_trace_list
    ...
    def create_coverage_from_drcov_list(self, drcov_list):
        ...
        for i, drcov_data in enumerate(drcov_list, 1):
            ...
            coverage = self.create_all_coverage(
                coverage_name,
                coverage_data,
                drcov_data
            )
    ...
    def coalesce_function_trace_blocks(self,blocks):
        """
        ...
        while blocks:
            ...
            block_start, block_size,func_id= blocks.pop(0)
            ...
            if sum(coalesced[-1]) < block_start:
                coalesced.append((block_start, block_size,func_id))
                continue
            ...
            coalesced[-1] = (coalesced[-1][0], (block_start + block_size) - coalesced[-1][0],coalesced[-1][2])

        # return the list of coalesced blocks
        return coalesced
    def get_module_file_name(self,mid):
    ...
    def get_module_base(self,mid):
        for module in self.all_modules:
            if module.id==mid:
                return module.base
        return "unknown module"
    def _normalize_function_trace_data(self, drcov_data,funcid_List):

        root_filename = self.metadata.filename
        coverage_blocks = drcov_data.get_blocks_by_module_function_trace(root_filename,funcid_List)
    ...
    def update_coverage(self, coverage_name, coverage_data, coverage_filepath=None,drcov_data=None):
        """
    ...
    def create_execute_trece_from_drcov_list(self, drcov_list):
        for i, drcov_data in enumerate(drcov_list, 1):
            # keep the user informed about our progress while loading coverage
            disassembler.replace_wait_box(
                "Normalizing and mapping coverage %u/%u" % (i, len(drcov_list))
