...
class DatabaseMetadata(object):
    ...
    def flatten_blocks(self, basic_blocks):
        ...
        return output
    def flatten_blocks_funtion_trace(self, basic_blocks):
            """
            Flatten a list of basic blocks (address, size) to instruction addresses.
            ...
            """
            output = {}
            for address, size ,func_id in basic_blocks:
                instructions = self.get_instructions_slice(address, address+size)
                if not output[func_id]:
                    output[func_id]=(instructions)
                else:
                    output[func_id].extend(instructions)
            return output
