...
本文主要分析DynamoRIO插桩的主要流程和实现原理,并附加相关demo分析
DynamoRIO通过将程序代码进行反复插桩执行构建了源程序代码与操纵代码之间的桥梁,使DynamoRIO的客户端编写者能够在更高的层面上驾驭原有的程序代码
...
```
DynamoRIO在初始化时构建的一个全局的路由表,负责路由所有的插桩事件和ibl表.其中code->fcache_enter 
